...
company name, location, and salary for each matching job.*/declare @minsal decimal(10,2)=68000declare @maxsal decimal(10,2)=90000select j.job_title,c.company_name,j.job_location,j.salary from Jobs as jjoin Companies as c on j.company_id=c.company_idwhere j.salary between @minsal and @maxsal/* 7. Write an SQL query that retrieves the job application history for a specific applicant. Allow a
parameter for the ApplicantID, and return a result set with the job titles, company names, and
application dates for all the jobs the applicant has applied to.*/declare @app_id int= 2select j.job_title, c.company_name, a.application_date
from Applications as a
join Jobs as j on a.job_id= j.job_id
join Companies as c on j.company_id = c.company_id
where a.applicant_id = @app_id/* 8. Create an SQL query that calculates and displays the average salary offered by all companies for
job listings in the "Jobs" table. Ensure that the query filters out jobs with a salary of zero.*/INSERT INTO Companies (company_name,Location)
VALUES ('New Developers', 'Delhi')INSERT INTO Jobs (company_id, job_title, job_description, job_location, salary, job_type, posted_date)
VALUES (4, 'Devops Engineer', 'Develop and maintain software applications.', 'Old delhi', null, 'Full-time', GETDATE())select avg(salary)as [avg_salary] from(select salary from Jobswhere salary is not null and salary>0) as q1/* 9. Write an SQL query to identify the company that has posted the most job listings. Display the
company name along with the count of job listings they have posted. Handle ties if multiple
companies have the same maximum count.*/INSERT INTO Jobs (company_id, job_title, job_description, job_location, salary, job_type, posted_date)
VALUES(2, 'Backend Engineer', 'Develop and maintain software applications.', 'Punjab', null, 'Full-time', GETDATE()),(4, 'Frontend Engineer', 'Develop and maintain software applications.', 'Mumbai', 78000.00, 'Full-time', GETDATE())select c.company_name, count(j.job_id) as[count_of_jobs] from Companies as cjoin Jobs as j on c.company_id=j.company_idgroup by c.company_namehaving count(j.job_id) =(select max(job_count) from(select count(j.job_id) as [job_count] from Jobsgroup by company_id) as q1)/*  10. Find the applicants who have applied for positions in companies located in 'CityX' and have at
